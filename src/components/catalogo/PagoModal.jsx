...
export default function PagoModal({ producto, onClose, onConfirm }) {
  ...
  useEffect(() => {
    if (metodo !== "qr" || !producto?.id) return;

    const channel = supabase
      .channel('pagos_channel')
      .on('postgres_changes', { 
        event: 'INSERT', 
        schema: 'public', 
        table: 'pagos_pos',
        filter: `catalogo_id=eq.${producto.id}`
      }, (payload) => {
        if (payload.new.estado === 'aprobado') {
          setPagoRealizado(true);
          setTimeout(() => onConfirm("mercadopago"), 2000);
        }
      })
      .subscribe();
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-md flex items-center justify-center p-6 z-50">
      <div className="bg-white rounded-[32px] p-10 max-w-lg w-full shadow-2xl animate-in zoom-in-95">
            ...
            <div className="flex flex-col items-center animate-in fade-in">
              {pagoRealizado ? (
                <>
                  ...
                  <h3 className="mt-6 text-3xl font-bold text-green-600">¡Pago realizado!</h3>
                </>
              ) : loadingQr ? (
                ...
                <>
                  ...
                  <p className="mt-6 text-blue-600 font-bold text-lg">Escanea para pagar</p>
                </>
              )}
            </div>
          ) : (
            <div className="flex flex-col items-center animate-in fade-in">
              <svg className="w-32 h-32 mb-4 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><circle cx="12" cy="12" r="10"/><path d="M16 8l-8 8M8 8l8 8"/></svg>
        ...
        <div className="space-y-4">
          {metodo && !pagoRealizado && (
            <button onClick={() => onConfirm(metodo)} className="w-full bg-pink-500 hover:bg-pink-600 text-white py-5 rounded-2xl font-bold text-lg transition-all active:scale-95">
              Finalizar Venta
            </button>
          )}
          <button onClick={metodo ? () => { setMetodo(null); setUrlPago(null); } : onClose} className="w-full text-gray-500 font-bold py-3 hover:text-gray-800 transition">
